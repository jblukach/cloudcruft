...
class CloudcruftStack(Stack):
    ...
    ### CHATBOT ###
        ...
        topic = _sns.Topic(
            self, 'topic',
            display_name = 'cloudcruft',
            topic_name = 'cloudcruft'
        )
    ...
    ### UNIQUE LAMBDA ###
        ...
        uniquelogs = _logs.LogGroup(
            self, 'uniquelogs',
            log_group_name = '/aws/lambda/'+unique.function_name,
            retention = _logs.RetentionDays.ONE_DAY,
            removal_policy = RemovalPolicy.DESTROY
        )

        uniquealarm = _cloudwatch.Alarm(
            self, 'uniquealarm',
            comparison_operator = _cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
            threshold = 0,
            evaluation_periods = 1,
            metric = unique.metric_errors(
                period = Duration.minutes(1)
            )
        )

        uniquealarm.add_alarm_action(
            _actions.SnsAction(topic)
        )

        uniqueevent = _events.Rule(
            self, 'uniqueevent',
